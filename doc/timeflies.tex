...
\DefineVerbatimEnvironment{inputplain}{Verbatim}
   {baselinestretch=0.8,
    labelposition=topline}
...
Software Version \input{version.txt}
\end{center}
\newpage
\tableofcontents
\newpage
...
This lets you define the times you need to work on the different days of the week. All weekdays of one single working time arrangement have to be given in one \verb:must-hours: statement. Multiple \verb:must-hours: statements can be used to change the work hours to e.g. reflect you going from full time to 80\% or back.

This example defines a standard 40 hour working week:
\begin{inputfile}
must-hours mon=8.0 tue=8.0 wed=8.0 thu=8.0 fri=8.0
\end{inputfile}

This example defines an 80\% week with Mondays off:
\begin{inputfile}
...
If a \verb:must-hours: statement appears inside a day block, the given working times are applicable from the date of that day block onwards until the date of the chronologically next day block with a \verb:must-hours: statement (which will be applicable from then onwards).

If a \verb:must-hours: statement appears outside a day block (before the very first day block in the input), then the given working times are applicable until the chronologically first day block with a \verb:must-hours: statement (which will be applicable from then onwards).

In above explanation of applicability the word \emph{chronologically} is very important because day blocks in \timeflies\ do not need to appear in chronological order in the input. The applicability of the \verb:must-hours: statements is determined by the chronological order, though, which is not necessarily the same as the statements' order of appearance in the input file(s).

If a day block has no applicable \verb:must-hours: statement, then the default
\begin{inputplain}
must-hours mon=8.0 tue=8.0 wed=8.0 thu=8.0 fri=8.0
\end{inputplain}
is assumed.
